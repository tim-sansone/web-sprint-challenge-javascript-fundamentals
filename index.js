...
// Functions are able to look upward/outward into the larger context that contains them in order to find variables and other functions. This includes the global context and other functions in which they are nested.
// In this case, since nestedFunction is within myFunction, and internal is ALSO within myFunction, nestedFunction is able to reach up into myFunction to find the definition of that variable.
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(data){
    const newArray = data.map(animal => animal.animal_name.toLowerCase())
    return newArray;
  }
  ...
  */
  ...
  function lowPopulationAnimals(data){
    const endangered = data.filter(animal => animal.population < 5);
    return endangered;
  }
  ...
  */
  ...
  function USApop(data){
    const totPop = data.reduce((counter, animal) => counter += animal.population, 0);
    return totPop;
  }
